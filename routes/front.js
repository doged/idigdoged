...
function withdrawp(req, res) {
	...
	if (typeof address !== 'string'
		...
		|| !addresses(address)) return res.render('withdraw', {
			withdraw: id
			, message: 'That address doesn\'t look like a DogecoinDark address. Please make sure you copied the whole thing. If you\'re sure it\'s right, please feel free to contact us using the link at the bottom of the page.'
		});
		...
		redis.get('balance::email::' + email, function(err, amount) {
			...
			redis.rpush('withdraw', JSON.stringify(transaction), function(err) {
				...
				redis.incrbyfloat('balance::email::' + email, -amount, function(err) {
					...
					// All good
					res.render('withdraw', {
						message: 'It all checks out! Your withdrawal has been added to the queue, and should be processed within 24 hours. Thanks again for using I Dig DogeD!'
					});
				});
			});
		});
